...
```
### 🚧Creating a wallet
NOTE: write your wallet name
```
0gchaind keys add write-wallet-name --eth
```
- import old wallet
```
0gchaind keys add wallet --eth --recover
```
### 🚧Get wallet home address
```
echo "0x$(0gchaind debug addr $(0gchaind keys show wallet -a) | grep hex | awk '{print $3}')"
```
> evm scan :   https://scan-testnet.0g.ai
> Getting private key for EVM, adding mm to the same wallet. Write your name in your wallet.
```
...
NOTE: Get your roles from Discord so that the channels can be opened.

![image](https://github.com/Core-Node-Team/Testnet-TR/assets/91562185/2b73ebc0-0880-4237-be41-aeb739f2d325)


NOTE: write your moniker name in the wallet name
```
0gchaind tx staking create-validator \
  --amount=1000000ua0gi \
  --pubkey=$(0gchaind tendermint show-validator) \
  --moniker=write-moniker \
  ...
  -y
```
### Test file upload attempt

https://scan-testnet.0g.ai/tool


- We go to the address and connect our wallet. Adds the network automatically. Then we select any small image file and upload it and give approval.
![image](https://github.com/Core-Node-Team/Testnet-TR/assets/91562185/36d5d3ee-85a2-4d7a-ae56-8784b0fa8c1c)


### Self-delegated
NOTE: Write the wallet name in the wallet section in 2 places. Write the number in the amount section. If you want 1 grain, delete the amount and write 1, otherwise it will drop 1 zero or so on.
```bash
